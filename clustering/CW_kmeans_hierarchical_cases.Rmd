...
```{r}
mykmeans <- kmeans(x = polydata, centers = 2)
```
...
```{r}
polydata %>%
  mutate(cluster = factor(mykmeans$cluster)) %>%
  ggplot() +
...
```{r}
polydata %>%
  mutate(hc_cluster = comp2) %>%
  ggplot() +
  geom_point(aes(x = x, y = y, color = hc_cluster))
